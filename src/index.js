...
io.on('connection', (socket) => {
  ...
  socket.on('move', (data, callback) => {
    ...
    const kills = currentGame.rouletteKills[color];

    if (move.piece !== 'k' && rouletteRoll === 1 && kills < 5) {
      chess.remove(to);
      wasRouletteKill = true;
      currentGame.rouletteKills[color] += 1;
    }

    currentGame.turn = currentGame.turn === 'w' ? 'b' : 'w';

    // --- THIS IS THE IMPORTANT CHANGE (added movedPieceType: move.piece) ---
    io.to(gameID).emit('opponentMove', {
      ...
      captured: wasRouletteKill && targetPiece
        ...
        : null,
      rouletteKills: currentGame.rouletteKills,
      movedPieceType: move.piece // <--- ADDED FOR CLIENT BUGFIX
    });
